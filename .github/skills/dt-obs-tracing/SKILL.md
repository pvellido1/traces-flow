...
- **Use `getNodeName()`**: Simplest way to add service names. This function only needs one parameter. Example: fieldsAdd service_name = getNodeName(dt.smartscape.service)
...
## Service error rate metrics

Don't use the metric dt.service.request.failure_rate because it doesn't exist in Grail. Instead use dt.service.request.failure_count. You can calculate the rate comparing dt.service.request.count and dt.service.request.failure_count